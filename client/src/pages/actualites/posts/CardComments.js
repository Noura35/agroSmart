...
const CardComments = ({post}) => {
  ...
  const handleComment = (e) => {
    e.preventDefault();
    if (text) {
      dispatch(addComment(post._id, userData._id, text, userData.nom))
  ...
  return (
    <div className='comments-container'>
      {post.comments.map((comment) => {
        return (
          <div className={comment.commenterId === userData.id ? 'comment-container client' : 'comment-container'} key={comment._id}>
      ...
      {userData._id && (
        <form action='' onSubmit={handleComment} className="comment-form">
          ...
          <input type="text" name="text" onChange={(e) => setText(e.target.value)} value={text} placeholder="Laissez un commentaire ..." />
